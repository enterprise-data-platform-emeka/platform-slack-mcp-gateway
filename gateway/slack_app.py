...
from gateway.analytics import AnalyticsAgentError, AnalyticsMCPTool
...
def build_app(config: Config, tool: AnalyticsMCPTool) -> App:
    ...
    def handle_app_mention(event: dict, say, client) -> None:  # type: ignore[no-untyped-def]
        ...
        if not allowed_channel(channel_name, config.allowed_channels):
            ...
            return
        _answer(event=event, say=say, tool=tool, slack_sessions=slack_sessions)

    @app.event("message")
    def handle_direct_message(event: dict, say) -> None:  # type: ignore[no-untyped-def]
...
def _answer(
    *,
    ...
    try:
        ...
        say(text=format_error(str(exc)), thread_ts=event.get("thread_ts") or event.get("ts"))
        return

    if result.session_id:
        slack_sessions[slack_thread_key] = result.session_id

    say(text=format_answer(result), thread_ts=event.get("thread_ts") or event.get("ts"))
